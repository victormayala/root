...
if ( ! function_exists( 'woodmart_shortcode_single_product_fbt_products' ) ) {
	...
	function woodmart_shortcode_single_product_fbt_products( $settings ) {
		...
		Main::setup_preview();
		?>
		<div class="wd-single-fbt wd-wpb<?php echo esc_attr( $wrapper_classes ); ?>"><?php Frontend::get_instance()->get_bought_together_products( $settings ); // Must be in one line. ?></div>
		<?php
